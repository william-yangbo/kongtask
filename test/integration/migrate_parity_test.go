...
func TestMigrate_InstallSchema_SecondMigrationDoesNoHarm(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, 11, migrationCount, "Should have exactly 11 migration records (including revision tracking, index optimization, JobKeyMode, cron support, pgcrypto removal, and complete_job removal)")
...
func TestMigrate_WithExistingSchema(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, 11, initialMigrationCount, "Should have 11 migration records after first migration")
	...
	require.NoError(t, err)
	assert.Equal(t, 11, finalMigrationCount, "Migration count should remain 11 after repeated migrations")
